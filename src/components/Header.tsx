...
const Header = () => {
  ...
  return (
    <nav key={language} className={`header-glass fixed top-0 z-50 shadow-soft w-full ${isRTL ? 'rtl' : 'ltr'}`}>
        ...
        <div className={`flex justify-between items-center h-20 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`lg:hidden flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
            <button
              onClick={toggleLanguage}
              disabled={isTransitioning}
              className={`px-3 py-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2 text-xs font-semibold ${
                isTransitioning ? 'opacity-75 cursor-not-allowed' : ''
              }`}
              aria-label={`Switch to ${language === 'en' ? 'Arabic' : 'English'}`}
            >
      ...
      {/* Mobile Navigation */}
      {mobileOpen && (
        <div className="lg:hidden mobile-menu px-4 pt-2 pb-6 space-y-1" aria-label="Mobile menu">
          ...
          <div className="pt-4 border-t border-gray-200 space-y-3">
            ...
            {mobileRegisterOpen && (
              <div className="space-y-2 ml-4">
                <a href="/getpetmicrochipped" className="block bg-orange-100 text-orange-800 py-2 px-4 rounded-lg font-medium">
                  {t('nav.registerPet')}
                </a>
                <a href="/vetregister" className="block bg-orange-100 text-orange-800 py-2 px-4 rounded-lg font-medium">
                  {t('nav.veterinaryRegister')}
                </a>
              </div>
            )}
          ...
          {mobileLoginOpen && (
            <div className="space-y-2 ml-4">
              <a href="/pologin" className="block bg-blue-100 text-blue-800 py-2 px-4 rounded-lg font-medium">
                {t('nav.petOwnerLogin')}
              </a>
              <a href="/vlogin" className="block bg-blue-100 text-blue-800 py-2 px-4 rounded-lg font-medium">
                {t('nav.veterinaryLogin')}
              </a>
              <a href="/alogin" className="block bg-gray-100 text-gray-800 py-2 px-4 rounded-lg font-medium">
                {t('nav.adminLogin')}
              </a>
            </div>
          )}
...
export default Header;      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, []);
        ...
        <div className={`flex justify-between items-center h-20 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`hidden lg:flex items-center ${isRTL ? 'space-x-reverse space-x-3' : 'space-x-3'}`}>
            ...
            <button
              ...
              disabled={isTransitioning}
              className={`
                w-[100px] h-10 px-3 py-2 rounded-lg
